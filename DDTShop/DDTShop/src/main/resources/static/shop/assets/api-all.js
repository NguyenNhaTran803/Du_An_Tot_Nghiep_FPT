...
$(document).ready(function() {
	...
	function showCategory() {
		axios({
			...
			url: baseUrl + "/ddt/layout/category"
		})
			.then(response => {
				response.data.forEach(item => {
					var itemMenu = document.createElement('li');
					itemMenu.innerHTML = `<a href="/ddt/shop?category_id=` + item.id + `">` + item.name + `</a>`;

					menuData.appendChild(itemMenu);

					var itemMenuMobile = document.createElement('li');
					itemMenuMobile.innerHTML = `<a href="/ddt/shop?category_id=` + item.id + `">` + item.name + `</a>`;

					menuMobileData.appendChild(itemMenuMobile);

					var itemMenuMobileTab = document.createElement('li');
					itemMenuMobileTab.innerHTML = `<a href="/ddt/shop?category_id=` + item.id + `">` + item.name + `</a>`;

					menuMobileTabData.appendChild(itemMenuMobileTab);

					var itemMenuTitleCategory = document.createElement('li');
					itemMenuTitleCategory.innerHTML = `<a href="/ddt/shop?category_id=` + item.id + `">` + item.name + `</a>`;
	...
	function showBrand() {
		axios({
			method: 'GET',
			contentType: "application/json",
			url: baseUrl + "/ddt/layout/brand"
		})
			.then(response => {
				response.data.forEach(item => {
					var itemTitleBrand = document.createElement('li');
					itemTitleBrand.innerHTML = `<a href="/ddt/shop?brand_id=` + item.id + `">` + item.name + `</a>`;

					menuTitleBrandData.appendChild(itemTitleBrand);

					var itemTitleMobileBrand = document.createElement('li');
					itemTitleMobileBrand.innerHTML = `<a href="/ddt/shop?brand_id=` + item.id + `">` + item.name + `</a>`;
	...
	function getCountCart() {
		axios({
			method: 'GET',
			contentType: "application/json",
			url: baseUrl + "/ddt/layout/get-cart-items"
		})
			.then(response => {
				var totalPrice = 0;
				...
				menuCart.innerHTML = `
					<div class="dropdown-menu dropdown-menu-right">
						...
						<div class="dropdown-cart-action">
							<a href="/ddt/cart" class="btn btn-primary">Giỏ hàng</a> 
							<a href="/ddt/check-out" class="btn btn-outline-primary-2">
						...
						productItem.innerHTML += `
									<div class="product-cart-details">
										<h4 class="product-title">
											<a href="/ddt/product-detail?id=`+ response.data[i].id + `">` + response.data[i].name + `</a>
										</h4>
									...
									<figure class="product-image-container">
										<a href="/ddt/product-detail?id=`+ response.data[i].id + `" class="product-image"> 
...
function removeCartItem(id, name) {
	Swal.fire({
		...
		if (result.isConfirmed) {
			axios.delete(baseUrl + '/ddt/cart/remove/' + id)
