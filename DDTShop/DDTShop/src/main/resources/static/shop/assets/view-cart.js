...
function updateValueDisplay(id) {
	...
	axios.post(baseUrl + '/ddt/cart/update?id=' + id + "&sst=" + qty)
	...
	axios.get(baseUrl + '/ddt/cart/totail-cart')
		.then(function(response) {
			let totailCart = 0;
	...
	axios.get(baseUrl + '/ddt/cart/totail-cart')
		.then(function(response) {
			...
			if (ship === '1') {
				...
				totailViewOrder = totailViewOrder + 30000;
				shipping = 2;
			} else {
				totailViewOrder = totailViewOrder + 0;
			}
			axios.post(baseUrl + '/ddt/cart/shipping?ship=' + shipping);
