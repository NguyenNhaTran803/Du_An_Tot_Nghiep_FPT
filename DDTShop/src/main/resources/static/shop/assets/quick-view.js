...
function formatVND(n, currency) {
	return n.toFixed(0).replace(/(\d)(?=(\d{3})+\.)/g, '$1,').replace('.00', '') + currency;
}
...
function quickView(id) {
	axios({
		...
		url: baseUrl + "/ddt/product-detail/quick-view/" + id
	})
			...
			if (response.data.discount <= 0 || response.data.startDayDiscount == null || response.data.endDayDiscount == null) {
				...
				if (now >= startDate && now <= endDate) {
					priceDiscount = `<h3 class="product-price"><span class="new-price">`
						+ formatVND(response.data.price - (response.data.price * response.data.discount * 0.01), ' VND').replace('.00', '') +
						`</span><span class="old-price"><del>` + formatVND(response.data.price, ' VND').replace('.00', '') + `<del></span></h3>`;
				} else {
					priceDiscount = `<h3 class="product-price"><span class="new-price">` + formatVND(response.data.price, ' VND') + `</span></h3>`;
				}
			}
...
async function viewImages(id) {
	images = [];
	const response = await axios.get(baseUrl + "/ddt/product-detail/quick-view/view-image/" + id);
