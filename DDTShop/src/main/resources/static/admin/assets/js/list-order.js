...
function viewOrder(id) {
	...
	axios({
		...
		.then(function(response) {
			...
			// Convert chuỗi ngày tháng từ API thành đối tượng Date
			var createDate = new Date(response.data.createdDate);

			// Định dạng ngày tháng theo ý muốn
			var formattedCreatedDay = `${createDate.toLocaleDateString()} ${createDate.toLocaleTimeString()}`;

			// Hiển thị ngày tháng đã định dạng trong modal
			viewDay.innerText = formattedCreatedDay;
...
// hàm gọi phương thức file excel
function exportOrder(id) {
	// Gọi API để lấy dữ liệu hóa đơn từ server
	axios({
		method: 'GET',
		contentType: "application/json",
		url: baseUrl + "/orders/export/" + id,
		responseType: 'blob', // Lưu ý: sử dụng 'blob' để nhận dữ liệu dưới dạng binary
	})
		.then(response => {
			...
			// Thêm link vào DOM và kích hoạt sự kiện click
		...
		.catch(error => {
			console.error('Xuất hóa đơn lỗi:', error);
		});
}
